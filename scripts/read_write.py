...
PT = pd.read_csv(StringIO("""1,Hydrogen,H,1.007,0,1,1,1,1,gas,,yes,,yes,,Nonmetal,0.79,2.2,13.5984,8.99E-05,14.175,20.28,3,Cavendish,1766,14.304,1,1
...
"""), names=["AtomicNumber","Element","Symbol","AtomicMass","NumberofNeutrons","NumberofProtons","NumberofElectrons","Period","Group","Phase","Radioactive","Natural","Metal","Nonmetal","Metalloid","Type","AtomicRadius","Electronegativity","FirstIonization","Density","MeltingPoint","BoilingPoint","NumberOfIsotopes","Discoverer","Year","SpecificHeat","NumberofShells","NumberofValence"])

def get_symbol_from_mass(m):
    return PT.loc[np.abs(PT['AtomicMass'] - m) < .1,'Symbol'].values[0]
    ...
    if alpha != 90 or beta != 90 or gamma != 90:
        for it,struc in enumerate(structure):
            ...
            if np.sum(struc.get_velocities())==0:
                ...
                for i,(at,pos,vel) in enumerate(zip(struc.get_chemical_symbols(), struc.positions, struc.get_velocities())):
                    x,y,z = pos
                    vx,vy,vz = vel
                    sys.stdout.write(f"{i+1:>4d} {at:>2s} {x:15.9e} {y:15.9e} {z:15.9e} {vx:15.9e} {vy:15.9e} {vz:15.9e}\n")
    else:
        for it,struc in enumerate(structure):
...
def write_inputdata(atoms, comments, rmFE = False):
    ...
    for atom,comment in zip(atoms,comments):
        ...
        for at,pos,(fx,fy,fz) in zip(atom.get_chemical_symbols(), atom.positions, forces):
            x,y,z = pos*cdist # convert Å to Bohr
            sys.stdout.write(f"atom {x:22.14e} {y:22.14e} {z:22.14e} {at:s} {'0.0':s} {'0.0':s} {fx:22.14e} {fy:22.14e} {fz:22.14e}\n")
    ...
    with open(filename, 'r') as f:
        for line in f:
            ...
            if 'atom' in line:
                ...
                forces.append([float(fx)/cforce, float(fy)/cforce, float(fz)/cforce])
            if 'energy' in line:
...
def compute_velocities(atoms, dt=1.0):
    ...
    """
    if len(atoms)<2:
        sys.stderr.write(f"{bold}{orange}\nVelocities could not be computed!\nAt least two images are necessary to compute them...{normal}\n")
        return atoms
    if len(atoms)>=3:
        sys.stderr.write("Computing the velocities using the Verlet algorithm.\n")
        ...
        sys.stderr.write("Computing the velocities using the finite difference method between the two images.\n")
        cell = atoms[0].cell
        inv_cell = np.linalg.inv(cell)
