...
def write_pdb(struct, name="", include_pbc_bonds=True, include_bonds=True):
    ...
    if include_bonds:
        connected = boron_bond_pairs(boron, cutoff=2.2, include_pbc=include_pbc_bonds)
...
def boron_bond_pairs(boron, cutoff=2.2, include_pbc=True):
    if len(boron) < 2:
        return np.empty((0, 2), dtype=int)

    cutoff = float(cutoff)
    # ASE neighbor lists scale much better than explicit O(N^2) pair scans.
    i_idx, j_idx, offsets = neighbor_list("ijS", boron, cutoff)
    if len(i_idx) == 0:
        return np.empty((0, 2), dtype=int)
    ...
    pairs = np.column_stack((i_idx[keep], j_idx[keep]))
